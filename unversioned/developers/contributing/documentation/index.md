...
3. The {doc}`examples gallery <napari:gallery>` is generated from Python source files and changes to the gallery should be made to the
...
5. The {ref}`API reference documentation <api>` is autogenerated from the napari source code docstrings.
...
- {ref}`**Getting started** <getting-started>` (in [`napari/docs/tutorials/fundamentals`](https://github.com/napari/docs/tree/main/docs/tutorials/fundamentals)):
...
The {doc}`**Examples gallery** <napari:gallery>` sources are in the [main `napari/napari` repository](https://github.com/napari/napari/tree/main/examples)
...
development installation of napari installed. {doc}`Examples <napari:gallery>`
...
(add-examples)=
## Adding examples to the {doc}`Gallery <napari:gallery>`

All of the examples in the {doc}`examples Gallery <napari:gallery>` are Python scripts that
